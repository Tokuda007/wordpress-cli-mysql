...
?>
		...
		<style type="text/css">
			...
			.primary-menu ul li ul.sub-menu {
				background-color: <?php echo esc_attr(get_theme_mod('navigation_background_color')); ?>;
			}
			...
			.mobile-menu-active .smenu-hide {
				color: <?php echo esc_attr(get_theme_mod('navigation_text_color')); ?>;
			}
			...
			.primary-menu .pmenu,
			.super-menu {
				border-color: <?php echo esc_attr(get_theme_mod('navigation_border_color')); ?>;
				border-bottom-color: <?php echo esc_attr(get_theme_mod('navigation_border_color')); ?>;
			}
			...
			#secondary .widget h5,
			#secondary .widget h6 {
				color: <?php echo esc_attr(get_theme_mod('sidebar_headline_color')); ?>;
			}
			...
			#secondary span.sub-arrow {
				color: <?php echo esc_attr(get_theme_mod('sidebar_link_color')); ?>;
			}
			...
			.widget time.rpwe-time.published {
				color: <?php echo esc_attr(get_theme_mod('sidebar_text_color')); ?>;
			}
			...
			.featured-sidebar .search-field {
				border-color: <?php echo esc_attr(get_theme_mod('sidebar_border_color')); ?>;
			}
			...
			.footer-column-three caption {
				color: <?php echo esc_attr(get_theme_mod('footer_text_color')); ?>;
			}

			<?php if (get_theme_mod('toggle_tagline') == '1') : ?>p.site-description {
				display: block;
			}
			...
			.swidgets-wrap h3,
			.post-data-text {
				background: <?php echo esc_attr(get_theme_mod('website_background_color')); ?>;
			}

			.site-title a,
			.site-description {
				color: <?php echo esc_attr(get_theme_mod('header_logo_color')); ?>;
			}

			.sheader {
				background: <?php echo esc_attr(get_theme_mod('header_background_color')); ?>
			}
		</style>
	<?php }
...
function custom_admin_alert_script()
{
...
// add_action('admin_footer', 'custom_admin_alert_script');

function save_content_length_as_custom_field($post_id)
{
	// Kiểm tra xem người dùng có quyền lưu bài viết hay không
	...
	// Lưu độ dài nội dung vào custom field
	update_post_meta($post_id, 'content_length', $content_length);
}

add_action('save_post', 'save_content_length_as_custom_field');
/**
